...
function returnToMenu() { 
    ...
    if(lobbyMusic.paused) {
        if (lobbyMusic.duration && isFinite(lobbyMusic.duration)) {
            lobbyMusic.currentTime = Math.random() * lobbyMusic.duration;
        }
        lobbyMusic.play().catch(e => console.log("A reprodução da música foi bloqueada.", e));
    }
}
...
function loadSettings() {
    ...
    const defaults = { fps: 60, effectsOn: true, uiOpacity: 100, aimOpacity: 40, musicVolume: 80, sfxVolume: 100 };
...
function applySettings() {
    ...
    clickSound.volume = gameSettings.sfxVolume / 100;
}
...
function updateSettingsUI() {
    ...
    sfxVolumeSlider.value = gameSettings.sfxVolume; sfxVolumeValue.textContent = `${gameSettings.sfxVolume}%`;
}
